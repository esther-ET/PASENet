...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None):
       ...
       # print(augmentor_configs) ----------------------####################---------------------
       #  aug_config_list = augmentor_configs if isinstance(augmentor_configs, list)  \
       #      else augmentor_configs['AUG_CONFIG_LIST']

       #------------------------------------------------------------------------------------------
        aug_config_list =  [{'NAME': 'gt_sampling', 'USE_ROAD_PLANE': True, 'DB_INFO_PATH': ['dense_dbinfos_train_clear.pkl'], 'PREPARE': {'filter_by_min_points': ['Car:5', 'Pedestrian:5', 'Cyclist:5'], 'filter_by_difficulty': [-1]}, 'SAMPLE_GROUPS': ['Car:20', 'Pedestrian:15', 'Cyclist:15'], 'NUM_POINT_FEATURES': 5, 'DATABASE_WITH_FAKELIDAR': False, 'REMOVE_EXTRA_WIDTH': [0.0, 0.0, 0.0], 'LIMIT_WHOLE_SCENE': True},
                            {'NAME': 'random_world_flip', 'ALONG_AXIS_LIST': ['x']},
                            {'NAME': 'random_world_rotation', 'WORLD_ROT_ANGLE': [-0.78539816, 0.78539816]},
                            {'NAME': 'random_world_scaling', 'WORLD_SCALE_RANGE': [0.95, 1.05]}]          # DB_INFO_PATH   'dense_dbinfos_train_clear.pkl'  ???????????
       #------------------------------------------------------------------------------------------
        for cur_cfg in aug_config_list:
            #print(cur_cfg)-----------------------------------------------------------------------------------
            # print(cur_cfg['DB_INFO_PATH'])
