...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None, cur_class=None):
        ...
        for db_info_path in sampler_cfg['DB_INFO_PATH']:
            ...
            #self.root_path = f'/home/ubuntu/wyt/codes/LiDAR_snow_sim/lib/OpenPCDet/data/dense'
            #db_info_path = self.root_path.resolve()/db_info_path#os.path.join(self.root_path,db_info_path)#self.root_path.resolve()/db_info_path #self.root_path
            db_info_path = '/home/ubuntu/wyt/codes/LiDAR_snow_sim/lib/OpenPCDet/data/dense/'+db_info_path
            print('lalallalalalalalalalalalalal')
            print(db_info_path)
            with open(str(db_info_path), 'rb') as f:              # dense-dataset.py 369 line
                infos = pickle.load(f)
        ...
        #     self.db_infos = getattr(self, func_name)(self.db_infos, val)                    #注掉了bu?
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(
                    [-1, self.sampler_cfg.get('NUM_POINT_FEATURES')])             #加了个get（这个属性）
        ...
        large_sampled_gt_boxes = box_utils.enlarge_box3d(
            sampled_gt_boxes[:, 0:7], extra_width=self.sampler_cfg.get('REMOVE_EXTRA_WIDTH')
        )
