...
class TANetHead(AnchorHeadTemplate):                                          #class PSA(nn.Module):
    def __init__(self,  # 初始化方法，用于定义类的属性和初始化操作
                 ...
                 num_input_filters=384,  # 输入特征的通道数，默认为128  384
        ...
        self.block1 = Sequential(
            ...
            # DeformConv2d(num_input_filters, num_filters[0], 3, stride=layer_strides[0], padding=1, bias=False,
        ...
        for i in range(layer_nums[0]):  # 遍历block1的所有卷积层,使用for循环添加了layer_nums[0]个相同结构的卷积层、批标准化层和ReLU激活函数层，以构建深度网络结构
            ...
            # self.block1.add(
            #     Conv2d(num_filters[0], num_filters[0], 3, padding=num_padding[i+1], dilation=num_dilation[i+1]))  # 卷积层
            self.block1.add(BatchNorm2d(num_filters[0]))  # 批标准化
            self.block1.add(nn.ReLU())  # 激活函数

        # self.block1.add(DeformConv2d(num_filters[0], num_filters[0], 3, stride=layer_strides[0], padding=1, bias=False,
        #                 offset_groups=1, with_mask=True))
        ...
        # self.block2.add(DeformConv2d(num_filters[1], num_filters[1], 3, stride=layer_strides[0], padding=1, bias=False,
        #                              offset_groups=1, with_mask=True))
        ...
        # self.block3.add(DeformConv2d(num_filters[2], num_filters[2], 3, stride=layer_strides[0], padding=1, bias=False,
        #                              offset_groups=1, with_mask=True))
    ...
    def forward(self, data_dict):
        ...
        # in_channels = spatial_features_2d.shape[1]
        # conv = nn.Conv2d(in_channels, 128, kernel_size=3, stride=1, padding=1)
        # conv = conv.cuda()
        # new_spatial_features_2d = conv(spatial_features_2d)
        # print('new_spatial_features_2d_is', new_spatial_features_2d.shape)  # ([4, 128, 248, 216])
