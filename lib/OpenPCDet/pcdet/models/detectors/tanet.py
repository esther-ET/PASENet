...
class TANet(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        # 调用父类的初始化方法，传入模型配置、类别数和数据集
...
# class PillarFeature_TANet(nn.Module):                       # PillarFeatureNet 增加了VoxelFeature_TA模块，作用是对points进行attention修正。
#     def __init__(self,
...
#         # 特征修饰是在不考虑柱状是否为空的情况下计算的。需要确保空柱状保持为零。
#         voxel_count = features.shape[1]
#         mask = PillarVFE.get_paddings_indicator(num_voxels, voxel_count, axis=0)
...
#
#         return features.squeeze()                                 # features is sq



#head is moved to dense document
