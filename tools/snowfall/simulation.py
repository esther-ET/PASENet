...
def augment(pc: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    # print('ground_distances ',ground_distances)
    # if len(ground_distances) !=0:
    p = np.polyfit(ground_distances, adaptive_noise_threshold, 2)     #def polyfit(x, y, deg, rcond=None, full=False, w=None, cov=False)
